...
hrR<-array(NA, dim=c(2,length(Years_m)-2, nsim))
hcW.au1<-array(NA, dim=c(2,length(Years_m)-2, nsim))
hcR.au1<-array(NA, dim=c(2,length(Years_m)-2, nsim))
hdcW<-array(NA, dim=c(2,length(Years_m)-2, nsim))
hdcR<-array(NA, dim=c(2,length(Years_m)-2, nsim))
...
hdoW<-array(NA, dim=c(2,length(Years_m)-2, nsim))
hdoR<-array(NA, dim=c(2,length(Years_m)-2, nsim))
hlW<-array(NA, dim=c(2,length(Years_m)-2, nsim))
hlR<-array(NA, dim=c(2,length(Years_m)-2, nsim))
for(y in 3:(length(Years_m))){ 
...
htW<-htR<-array(NA, dim=c(2,length(Years_m)-2, nsim))
for(y in 3:(length(Years_m))){ 
...
# from cohort+age to calendar years
hr_coastW<-array(NA, dim=c(2,length(Years_m)-2, nsim))
hr_coastR<-array(NA, dim=c(2,length(Years_m)-2, nsim))       #AU1

hr_coastW<- 1-exp(-(-log(1-hcW.au1)+ (-log(1-hdcW))))
hr_coastR<- 1-exp(-(-log(1-hcR.au1)+ (-log(1-hdcR))))       #AU1

hr_seaW<-array(NA, dim=c(2,length(Years_m)-2, nsim))
hr_seaR<-array(NA, dim=c(2,length(Years_m)-2, nsim))

hr_seaW<- 1-exp(-(-log(1-hdoW)+ -log(1-hlW)+ -log(1-htW)))
hr_seaR<- 1-exp(-(-log(1-hdoR)+ -log(1-hlR)+ -log(1-htR)))    
# wrangle
##########################
# Combined HR coast
# boxplot.bugs.df2 is used since chains-structure is not anymore in hrW
for(a in 1:2){ # grilse vs msw
  dfW<-boxplot.bugs.df2(hr_coastW, a ,1:(length(Years_m)-2))%>%
    mutate(age=a, Fishery="Coastal_tot", Type="Wild")
  ifelse(a>1, dfW2<-bind_rows(dfW2,dfW),dfW2<-dfW)
  
  dfR<-boxplot.bugs.df2(hr_coastR, a ,1:(length(Years_m)-2))%>%
...
# Combined HR sea
for(a in 1:2){ # grilse vs msw
  dfW<-boxplot.bugs.df2(hr_seaW, a ,1:(length(Years_m)-2))%>%
    mutate(age=a, Fishery="Sea_tot", Type="Wild")
  ifelse(a>1, dfW2<-bind_rows(dfW2,dfW),dfW2<-dfW)
  
  dfR<-boxplot.bugs.df2(hr_seaR, a ,1:(length(Years_m)-2))%>%
...
gp <- ggplot(df, aes(Year, group=Year))+
