...
# Smolt age composition observation model 

for(j in 1:n_age_obs){    
  for(i in 1:nages){
   
  	  N_age[j,i]<-round(age_prop[j,i]*n_aged[j])   #model_predicted numbers at age
     	a[j,i]<-mu[age_year[j], age_river[j],i]*eta_star[j]
  }
 age_prop[j,1:3]~ddirch(a[j,1:3])   #likelihood, make sure obs in 1st 5 years are not included (ES set to 0)

eta_star[j]<-n_aged[j]*(eta+1)/(eta+n_aged[j])-1
}
    ...
    # Year specific p-values
    ES[y,r] <- betas[r] * (p[1,y-1,r]*P2[y-1,r] + p[2,y-1,r]*P1[y-1,r] + p[3,y-1,r]*P0[y-1,r]) * A[r] / 1000
    EP[y,r] <- (p[1,y-1,r]*P2[y-1,r] + p[2,y-1,r]*P1[y-1,r] +   p[3,y-1,r]*P0[y-1,r]) * A[r] / 1000
    
    mu[y,r,1]<-p[3,y-1,r]*P0[y-1,r]/(p[1,y-1,r]*P2[y-1,r] + p[2,y-1,r]*P1[y-1,r] + p[3,y-1,r]*P0[y-1,r])  #0+  
    mu[y,r,2]<-p[2,y-1,r]*P1[y-1,r]/(p[1,y-1,r]*P2[y-1,r] + p[2,y-1,r]*P1[y-1,r] + p[3,y-1,r]*P0[y-1,r])  #1+
    mu[y,r,3]<-p[1,y-1,r]*P2[y-1,r]/(p[1,y-1,r]*P2[y-1,r] + p[2,y-1,r]*P1[y-1,r] + p[3,y-1,r]*P0[y-1,r])  #2+                                                                                                     
...
#for(y in 1:(years+1)){
for(y in 1:(years)){
S_Morrum[y]<- S[y,idx_Morrum]+S[y,idx_Morrum_low] # use index
...
Malpha[r]<-aalpha+balpha*(AL[r]-mean(AL[1:rivers]))/sd(AL[1:rivers])-0.5/Talpha
...
Mbetas[r]<-abetas+bbetas*(A[r]-mean(A[1:rivers]))/sd(A[1:rivers])+cbetas*(R[r]-mean(R[1:rivers]))/sd(R[1:rivers]) 
################################################################################
...
logit_p2prop[r] ~ dnorm(mu_p2prop, tau_p2prop) 
logit(p2prop[r]) <- logit_p2prop[r]
  
#alpha_parr[r] ~ dnorm(mu_alpha_parr, tau_alpha_parr)
#beta_parr[r] ~ dnorm(mu_beta_parr, tau_beta_parr)

mid_size[r]~dlnorm(mu_midsize,tau_midsize)
s[r]~dlnorm(mu_s,tau_s)

for(y in 1:years){ 

    # Probability smolt comes from 0+ parr (early smolt)
    #logit(p[3,y,r]) <- alpha_parr[r] + beta_parr[r] * size[y,r]  # actual size data 
   # logit(p[3,y,r]) <- alpha_parr[r] + beta_parr[r] * size_dev[y,r]  # size deviation from global mean
    logit(p[3,y,r]) <- (size[y,r]-mid_size[r])/s[r]
...
mu_midsize~dnorm(4.38,4)
tau_midsize~dunif(2,50)
mu_s~dnorm(2.30,4)
tau_s~dunif(2,50)
...
#mu_alpha_parr ~ dnorm(0,0.01) 
#tau_alpha_parr ~ dgamma(0.1,0.1) 
#mu_beta_parr ~ dnorm(0,0.01) 
#tau_beta_parr ~ dgamma(0.1,0.1) 
mu_p2prop ~ dnorm(0,0.01) 
tau_p2prop ~ dgamma(0.1,0.1) 
