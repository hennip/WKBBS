...
#Post-smolts (fishery abbreviations in name indicate pre-fishery abundance for that fishery)

    # Fish in the river in  the beginning of June in year 1 
    NrW[i,1,s] <- SmoltW[i,s]*survMps[i,1,s]  #*kE[i,1,1]    #survMps 1 1 mo

    # Coastal fish in the beginning of July in year 1      
    NcW[i,1,s] <- NrW[i,1,s]*survMps[i,5,s] #*kE[i,1,5]     #survMps 5 1 mo
    
   # Offshore salmon in the beginning of January in year 1 (calendar year 2) #survMps 3 6 mo
    NdoW[i,1,s] <- NcW[i,1,s]*survMps[i,3,s] #*kEc[i,1] 
   	
    # Offshore salmon in the beginning of February in year 1  (calendar year 2)
    NlW[i,1,s] <- NdoW[i,1,s]*survMps[i,4,s] #*kE[i,1,4]                  #survMps 4 1 mo
   																													
	# Offshore salmon in the beginning of April in year 1  (calendar year 2)
    NtW[i,1,s] <- NlW[i,1,s]*survMps[i,2,s] #*kE[i,1,2]		                    #survMps 2 2 mo
...
#            survMps[i,6,s]<-(1-HtW[i,1]*(p.rel[(i+1-1)]*p.mort+(1-p.rel[(i+1-1)])))* exp(-MpsW[i,s]/Tstep) 
    ...
    NccW[i,2,s]<-NtW[i,1,s]*survMps[i,6,s] #*kE[i,1,6]	       #survMps 6 1 mo
...
MR ~dlnorm(-2.3, 4.3)T(0.025,0.35)
   ...
   for(st in 1:stocks){                  
        ...
        logit_survMps[1,st] ~ dunif(-3,0)  #max 50%
