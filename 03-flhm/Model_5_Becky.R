...
for(s in 1:2){     #Torne, Simo
    for (i in 6:(m+proj_years)){
        ...
        NladderW_tot[i,s]<- (NladderW[(i-1),2,s]+NladderW[(i-2),3,s]+NladderW[(i-3),4,s]+NladderW[(i-4),5,s]+NladderW[(i-5),6,s]+NladderRsp[(i-1),2,s]+NladderRsp[(i-2),3,s]+NladderRsp[(i-3),4,s]+NladderRsp[(i-4),5,s]+NladderRsp[(i-5),6,s])*1000
...
for (i in 1:(m+proj_years)){             
    ...
    # Fish in the river in  the beginning of June in year 1 
    NrRsp[i,1,s] <- (SmoltRsp[i,s] + Parr[i,s])*survMpsR[i,1,AU[s]] #*kE[i,1,1]
   	
    # Coastal fish in the beginning of July in year 1      
    #NcRsp[i,1,s] <- NrRsp[i,1,s]*(1-HrW[i,1])*exp(-MpsR[i,AU[s]]*sealMort[i,1,1]/Tstep)*kE[i,1,5]
    NcRsp[i,1,s] <- NrRsp[i,1,s]*(1-HrW[i,1]*rivHR[(i+1-1),s])*exp(-MpsR[i,AU[s]]*sealMort[i,1,1]/Tstep) #*kE[i,1,5]
    
    # Offshore salmon in the beginning of January in year 1 (calendar year 2)
    NdoRsp[i,1,s] <- NcRsp[i,1,s]*survMpsR[i,3,AU[s]] #*kEc[i,1]		     
   	
    # Offshore salmon in the beginning of February in year 1  (calendar year 2)
    NlRsp[i,1,s] <- NdoRsp[i,1,s]*survMpsR[i,4,AU[s]] #*kE[i,1,4]	  

														   
	# Offshore salmon in the beginning of April in year 1  (calendar year 2)
    NtRsp[i,1,s] <- NlRsp[i,1,s]*survMpsR[i,2,AU[s]] #*kE[i,1,2]	
    ...
    NccRsp[i,2,s] <- NtRsp[i,1,s]*survMpsR[i,6,AU[s]] #*kE[i,(j-1),6]  	 
...
#Post-smolts
        ...
        NrR[i,1,s] <- SmoltR[i,s]*survMpsR[i,1,s] #*kE[i,1,1]	                                                      

    #    TrW[i,1,s] <- rel_W[i,s]*surv[i,1,1,1]*kE[i,1,1]       #tagged wild salmon releases AU1  Beginning of July
        TrR[i,1,s] <- rel_R[i,s]*survMpsR[i,1,s]#*kE[i,1,1]	     #tagged reared salmon NB s index stands for AU 
		    TrRsp[i,1,s] <- rel_Rsp[i,s]*survMpsR[i,1,s]#*kE[i,1,1]	 #tagged reared-spawned salmon NB s index stands for AU 

		# Coastal fish in the beginning of July in year 1
      
        NcR[i,1,s] <- NrR[i,1,s]*survMpsR[i,5,s] #*kE[i,1,5]
            
    #    TcW[i,1,s] <- TrW[i,1,s]*surv[i,1,5,1]*kE[i,1,5]	#rivHR change needed? surv.riv[i,1,AU_stock[s]]?? or leave as is....see process errors	     
        TcR[i,1,s] <- TrR[i,1,s]*survMpsR[i,5,s] #*kE[i,1,5]	      
        TcRsp[i,1,s] <- TrRsp[i,1,s]*(1-HrW[i,1])*exp(-MpsR[i,s]*sealMort[i,1,1]/Tstep) #*kE[i,1,5]	  
	
		# Offshore salmon in the beginning of January in year 1
      
 	 	    NdoR[i,1,s] <- NcR[i,1,s]*survMpsR[i,3,s]  #*kEc[i,1]	
       
    #    TdoW[i,1,s] <- TcW[i,1,s]*survc[i,1,1,s]*kEc[i,1]
        TdoR[i,1,s] <- TcR[i,1,s]*survMpsR[i,3,s] #*kEc[i,1] 
		    TdoRsp[i,1,s] <- TcRsp[i,1,s]*survMpsR[i,3,s]  #*kEc[i,1] 

		# Offshore salmon in the beginning of February in year 1
      
  	  	NlR[i,1,s] <- NdoR[i,1,s]*survMpsR[i,4,s]#*kE[i,1,4]	

															   
  #      TlW[i,1,s] <- TdoW[i,1,s]*survMpsR[i,4,s]  #*kE[i,1,4]	
        TlR[i,1,s] <- TdoR[i,1,s]*survMpsR[i,4,s]   #*kE[i,1,4]	
		    TlRsp[i,1,s] <- TdoRsp[i,1,s]*survMpsR[i,4,s]  #*kE[i,1,4]	
        
        		# Offshore salmon in the beginning of April in year 1
  	  	NtR[i,1,s] <- NlR[i,1,s]*survMpsR[i,2,s]   #*kE[i,1,2]
        TtR[i,1,s] <- TlR[i,1,s]*survMpsR[i,2,s]   #*kE[i,1,4]	
		    TtRsp[i,1,s] <- TlRsp[i,1,s]*survMpsR[i,2,s]  #*kE[i,1,4]		
   ...
   #     TdcW[i,1,s]<-0 
    ...
    NccR[i,2,s] <- NtR[i,1,s]*survMpsR[i,6,s] #*kE[i,(j-1),6]
    TccR[i,2,s] <-TtR[i,1,s]*survMpsR[i,6,s] #*kE[i,(j-1),2]	
    TccRsp[i,2,s] <-TtRsp[i,1,s]*survMpsR[i,6,s] #*kE[i,(j-1),2]	  	 
    ...
    for (j in 3:6){  
      ...
      #  TccW[i,j,s]<-TlW[i,(j-1),s]*surv[i,(j-1),6,1]*kE[i,(j-1),2]	
        TccR[i,j,s] <-TtR[i,(j-1),s]*surv[i,(j-1),6,2] #*kE[i,(j-1),2]	
		    TccRsp[i,j,s] <-TtRsp[i,(j-1),s]*surv[i,(j-1),6,2] #*kE[i,(j-1),2]	
     ...
     #   TdoW[i,j,s] <- TdcWI[i,j,s]*surv[i,j,3,1]*kE[i,j,3]
      ...
      #  TlW[i,j,s] <- TdoW[i,j,s]*surv[i,j,4,1]*kE[i,j,4]	 
        ...
        TtR[i,j,s] <- TlR[i,j,s]*surv[i,j,2,2] #*kE[i,j,4]	
			  TtRsp[i,j,s] <- TlRsp[i,j,s]*surv[i,j,2,2] #*kE[i,j,4]															 
...
for (i in 6:(m-1)){ 
    ...
    #sp_count[i,1]~dbin(p.detect[i,1],NrWtot[i,1]) 
    ...
    mu.sp[i,1]<-NrWtot[i,1]*p.detect[i,1]
    tau.sp[i,1]<-1/(NrWtot[i,1]*p.detect[i,1]*(1-p.detect[i,1]))
    
    sp_count[i,1]~dnorm(mu.sp[i,1],tau.sp[i,1]) # Torne Didson count 
    sp_countX[i,1]~dnorm(mu.sp[i,1],tau.sp[i,1])
    
    mu_ladder[i,1]<-log(NladderW_tot[i,1])-0.5/tau_ladder
    ladder_count[i,1]~dlnorm(mu_ladder[i,1],tau_ladder)
    ...
    mu_ladder[i,2]<-log(NladderW_tot[i,2])-0.5/tau_ladder
    ladder_count[i,2]~dlnorm(mu_ladder[i,2],tau_ladder)
    ...
    for(s in 3:stocks){
        ...
        mu.sp[i,s]<-NrWtot[i,s]*p.detect[i,s]
        tau.sp[i,s]<-1/(NrWtot[i,s]*p.detect[i,s]*(1-p.detect[i,s]))
...
#		NrR[(i-5),6,AUR[rs]]*RProp[(i-5),rs])*1000-CatchR[i,rs])
        ...
        NrRtot[i,rs]<-round(NrRtot.vul[i,rs]*(1-HRNrR[i,rs]))+1
        
          pTrap[i,rs]~dbeta(aTrap[rs],bTrap[rs])
        
   mu.trap[i,rs]<-NrRtot[i,rs]*pTrap[i,rs]
   tau.trap[i,rs]<-1/(NrRtot[i,rs]*pTrap[i,rs]*(1-pTrap[i,rs]))
...
MR ~dlnorm(-2.3, 4.3)T(0.025,0.35)
   ...
   sd_alpha_mps ~ dunif(0, 2)
   sd_delta_mps ~ dunif(0, 2)
   sd_epsilon_mps ~ dunif(0, 2)
   delta_mps[1] ~dnorm(0, sd_delta_mps^-2)
    
    for(j in 1:(AUS+1)){
      alpha_mps[j] ~ dnorm(0, sd_alpha_mps^-2)

    }
    for(st in 1:stocks){
 
      epsilon_mps[1, st] ~ dnorm(0, sd_epsilon_mps^-2)
      #logit_survMps[1,st] <- beta_mps[st] +  delta_mps[1] + alpha_mps[AU[st],1]  + epsilon_mps[1, st]
      logit_survMps[1,st] <- beta_mps[st] +  delta_mps[1] + alpha_mps[AU[st]]  + epsilon_mps[1, st]
      MpsW[1,st]<- -log((exp(logit_survMps[1,st])/(1+exp(logit_survMps[1,st]))))
    }
    
    #   posterior pred chech st 18 au 5
    epsilon_mps[1, 18] ~ dnorm(0, sd_epsilon_mps^-2)
    
    logit_survMps[1,18] <- alpha_mps[5] + beta_mps[18] + delta_mps[1] + epsilon_mps[1, 18]
    MpsW[1,18]<- -log((exp(logit_survMps[1,18])/(1+exp(logit_survMps[1,18]))))
    
      # for(j in 1:(AUS+1)){
      #   alpha_mps[j] ~ dnorm(rho_alpha_mps*alpha_mps[j], sd_alpha_mps^-2)
      # 
      # } 
      # 
    for(t in 2:m){

        delta_mps[t] ~dnorm(rho_delta_mps*delta_mps[t-1], sd_delta_mps^-2)
        

        for(st in 1:stocks){
            
            epsilon_mps[t, st] ~ dnorm(rho_epsilon_mps*epsilon_mps[t-1, st], sd_epsilon_mps^-2)

            #   Olmos et all 2019 based MPS estimation
            #   beta is stock-specific intercept 
            #   delta is global trend component
            #   alpha is au specific component
            #   epsilon is year and stock specific component
            
     
            logit_survMps[t,st]<- beta_mps[st] + delta_mps[t] + alpha_mps[AU[st]] +   epsilon_mps[t, st]
            
            MpsW[t,st]<- -log((exp(logit_survMps[t,st])/(1+exp(logit_survMps[t,st]))))
       
        }
        
        #   posterior pred check st 18 au 5
        epsilon_mps[t, 18] ~ dnorm(rho_epsilon_mps*epsilon_mps[t-1, 18], sd_epsilon_mps^-2)
        
        logit_survMps[t,18] <- alpha_mps[5] + beta_mps[18] + delta_mps[t] + epsilon_mps[t, 18]
        MpsW[t,18]<- -log((exp(logit_survMps[t,18])/(1+exp(logit_survMps[t,18]))))
    }
  

    mu_beta_mps ~ dunif(-3,0)  #5% to 50% surv
    sd_beta_mps ~ dunif(0, 1)
    for(st in 1:(stocks+1)){
      beta_mps[st] ~ dnorm(mu_beta_mps, sd_beta_mps^-2)

    }
    
    #   autoregression
    rho_alpha_mps   <- ilogit(rho_alpha_mps_N  *0.95)
    rho_delta_mps   <- ilogit(rho_delta_mps_N  *0.95)
    rho_epsilon_mps <- ilogit(rho_epsilon_mps_N*0.95)
    
    rho_alpha_mps_N   ~ dnorm(0, 1.75^-2)
    rho_delta_mps_N   ~ dnorm(0, 1.75^-2)
    rho_epsilon_mps_N ~ dnorm(0, 1.75^-2)
...
#  Rb<-(1-Reff_mu)/Reff_eta  
  ...
  logit_Reff_mu~dnorm(-0.69,0.29)
  tau_Reff~dlnorm(log(10),2) 
  ...
  for(i in 1:(m)){
        ...
        for(s in 1:AUS){
            survMpsR[i,1,s]<-exp(-MpsR[i,s]/Tstep) 		          #survmortW
            survMpsR[i,2,s] <- exp(-qlR[i,1]*El[i,1]) * exp(-2*MpsR[i,s]/Tstep) 	       #survlW  
            survMpsR[i,3,s] <- exp(-F_sea[i,1,1,s])* exp(-(6*MpsR[i,s]/Tstep))   	      #was survc
            survMpsR[i,4,s] <- exp(-qdR[i,1]*Edo[i,1]) * exp(-MpsR[i,s]/Tstep)         #survdoW
            survMpsR[i,5,s] <- (1-HrR[i,1])*exp(-MpsR[i,s]*sealMort[i,1,1]/Tstep)    #survrR
            survMpsR[i,6,s]<-(1-HtR[i,1])*exp(-MpsR[i,s]/Tstep) 
        }
        
    logit_RMps[i]~dnorm(logit_Reff_mu,tau_Reff)
    RMps[i]<-ilogit(logit_RMps[i])
    
#    RMps[i]~dbeta(Ra,Rb) 
    ReffectMps[i] <- (RMps[i] * 1.5)+1         

    for(s in 1:AUS){
      MpsR[i,s] <- MpsW[i,s] * ReffectMps[i]
 
    }             
    
    #surv[,1,,1] are now replaced in code by survMps
     #surv[,1,,1] are now replaced in code by survMpsR
    surv[i,1,1,1]<- 0.99
    surv[i,1,1,2]<- 0.99
...
#priors for spawner counting
  for(s in 1:stocks){
    ...
    #logit_mu_spawn[s]~dnorm(mu_mu_sp[s],tau_mu_sp[s])T(pdl[s],pdh[s]) 
    #logit_CV_spawn[s]~dnorm(mu_CV_sp[s],tau_CV_sp[s])T(pcvl[s],pcvh[s])  
    for(j in 1:2){  
          logit_mu_spawn[s,j]~dnorm(mu_mu_sp[s,j],tau_mu_sp[s,j])
          logit_CV_spawn[s,j]~dnorm(mu_CV_sp[s,j],tau_CV_sp[s,j])T(,0)  
    
          logit(mu_spawn[s,j])<-logit_mu_spawn[s,j] 
          logit(CV_spawn[s,j])<-logit_CV_spawn[s,j] 
    
          tau_spawn[s,j]<-(1-mu_spawn[s,j])^2/CV_spawn[s,j]^2
    }
    
    for(i in 1:prior_change[s]){
      logit_pdetect[i,s]~dnorm(logit_mu_spawn[s,1],tau_spawn[s,1])T(-4,4)
      logit(p.detect[i,s])<-logit_pdetect[i,s]
      ...
      surv_migr[i,s]~dbeta(alpha_migr[i,s],beta_migr[i,s])   
        ...
        for(i in (prior_change[s]+1):(m+5)){
      logit_pdetect[i,s]~dnorm(logit_mu_spawn[s,2],tau_spawn[s,2])T(-4,4)
      logit(p.detect[i,s])<-logit_pdetect[i,s]
