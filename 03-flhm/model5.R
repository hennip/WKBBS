   ...
   # Offshore salmon in the beginning of January in year 1 (calendar year 2)
    ...
    for (j in 3:6){  
        ...
        # Offshore salmon in the beginning of May in year 3-6  
		    ...
		    NccW[i,j,s]<-NtW[i,(j-1),s]*surv[i,(j-1),6,1] 
	 } 
   for (j in 2:6){ 
...
for (i in 1:(m+proj_years)){             
    ...
    #NcRsp[i,1,s] <- NrRsp[i,1,s]*(1-HrW[i,1]*rivHR[(i+1-1),s])*exp(-MpsR[i]*sealMort[i,1,1]/Tstep)
    NcRsp[i,1,s] <- NrRsp[i,1,s]*(1-HrW[i,1]*rivHR[(i+1-1),s])*exp(-MpsR[i,2]*sealMort[i,1,1]/Tstep)
    ...
    for (j in 2:6){ 
        ...
        NspRsp[i,j,s] <- NladderRsp[i,j,s]*(1-HrW[i,j]*rivHR[(i+j-1),s])* exp(-2*MR/Tstep)*surv_migr[(i+j-1),s]
        }
    }
}
...
#Post-smolts
        ...
        NrR[i,1,s] <- SmoltR[i,s]*surv[i,1,1,(s+1)]
    ...
    #    TrW[i,1,s] <- rel_W[i,s]*surv[i,1,1,1]*kE[i,1,1]       #tagged wild salmon releases AU1  Beginning of July
        ...
        #TrR[i,1,s] <- rel_R[i,s]*surv[i,1,1,2]
        TrR[i,1,s] <- rel_R[i,s]*surv[i,1,1,(s+1)]
		    # TrRsp[i,1,s] <- rel_Rsp[i,s]*surv[i,1,1,2]*kE[i,1,1]	 #tagged reared-spawned salmon NB s index stands for AU 
		    #TrRsp[i,1,s] <- rel_Rsp[i,s]*surv[i,1,1,2]	 #tagged reared-spawned salmon NB s index stands for AU 
		    TrRsp[i,1,s] <- rel_Rsp[i,s]*surv[i,1,1,(s+1)]	 #tagged reared-spawned salmon NB s index stands for AU 

		# Coastal fish in the beginning of July in year 1
      
        # NcR[i,1,s] <- NrR[i,1,s]*surv[i,1,5,2]*kE[i,1,5]
        #NcR[i,1,s] <- NrR[i,1,s]*surv[i,1,5,2]
        NcR[i,1,s] <- NrR[i,1,s]*surv[i,1,5,(s+1)]
    ...
    #    TcW[i,1,s] <- TrW[i,1,s]*surv[i,1,5,1]*kE[i,1,5]	#rivHR change needed? surv.riv[i,1,AU_stock[s]]?? or leave as is....see process errors	     
        ...
        #TcR[i,1,s] <- TrR[i,1,s]*surv[i,1,5,2]	      
        TcR[i,1,s] <- TrR[i,1,s]*surv[i,1,5,(s+1)]	      
        # TcRsp[i,1,s] <- TrRsp[i,1,s]*(1-HrW[i,1])*exp(-MpsR[i]*sealMort[i,1,1]/Tstep)*kE[i,1,5]	  
        #TcRsp[i,1,s] <- TrRsp[i,1,s]*(1-HrW[i,1])*exp(-MpsR[i]*sealMort[i,1,1]/Tstep)  
        TcRsp[i,1,s] <- TrRsp[i,1,s]*(1-HrW[i,1])*exp(-MpsR[i,s]*sealMort[i,1,1]/Tstep)  
    ...
    #    TdoW[i,1,s] <- TcW[i,1,s]*survc[i,1,1,s]*kEc[i,1]
        ...
        TdoR[i,1,s] <- TcR[i,1,s]*survc[i,1,2,s] 
		    # TdoRsp[i,1,s] <- TcRsp[i,1,s]*survc[i,1,2,s]*kEc[i,1] 
		    TdoRsp[i,1,s] <- TcRsp[i,1,s]*survc[i,1,2,s]
		...
		# Offshore salmon in the beginning of February in year 1
  	  	...
  	  	NlR[i,1,s] <- NdoR[i,1,s]*surv[i,1,4,(s+1)]
		...
		# Offshore salmon in the beginning of April in year 1
  	  	...
  	  	NtR[i,1,s] <- NlR[i,1,s]*surv[i,1,2,(s+1)]												   
													   
  #      TlW[i,1,s] <- TdoW[i,1,s]*surv[i,1,4,1]*kE[i,1,4]	
        # TlR[i,1,s] <- TdoR[i,1,s]*surv[i,1,4,2]*kE[i,1,4]	
        TlR[i,1,s] <- TdoR[i,1,s]*surv[i,1,4,2]
		    # TlRsp[i,1,s] <- TdoRsp[i,1,s]*surv[i,1,4,2]*kE[i,1,4]		
		    #TlRsp[i,1,s] <- TdoRsp[i,1,s]*surv[i,1,4,2]		
		    TlRsp[i,1,s] <- TdoRsp[i,1,s]*surv[i,1,4,(s+1)]		
    ...
    #    TdcWI[i,1,s]<-0 
     ...
     #   TspW[i,1,s]<-0
      ...
      #  TccW[i,j,s]<-TlW[i,(j-1),s]*surv[i,(j-1),2,1]*kE[i,(j-1),2]	
        ...
        TccR[i,j,s] <-TlR[i,(j-1),s]*surv[i,(j-1),2,2]	
		    # TccRsp[i,j,s] <-TlRsp[i,(j-1),s]*surv[i,(j-1),2,2]*kE[i,(j-1),2]	
		    TccRsp[i,j,s] <-TlRsp[i,(j-1),s]*surv[i,(j-1),2,2]	
     ...
     #   TdoW[i,j,s] <- TdcWI[i,j,s]*surv[i,j,3,1]*kE[i,j,3]
        ...
        TdoR[i,j,s] <- TdcRI[i,j,s]*surv[i,j,3,2]
        # TdoRsp[i,j,s]<- TdcRspI[i,j,s]*surv[i,j,3,2]*kE[i,j,3]
        TdoRsp[i,j,s]<- TdcRspI[i,j,s]*surv[i,j,3,2]
      ...
      #  TlW[i,j,s] <- TdoW[i,j,s]*surv[i,j,4,1]*kE[i,j,4]	 
        ...
        TlR[i,j,s] <- TdoR[i,j,s]*surv[i,j,4,2]	
			  # TlRsp[i,j,s] <- TdoRsp[i,j,s]*surv[i,j,4,2]*kE[i,j,4]	
			  TlRsp[i,j,s] <- TdoRsp[i,j,s]*surv[i,j,4,2]
      ...
      #  TcW[i,j,s] <- TdcW[i,j,s]*survdc[i,j,1,s]*kEdc[i,j]	         
        ...
        TcR[i,j,s] <- TdcR[i,j,s]*survdc[i,j,2,s]	
        # TcRsp[i,j,s] <- TdcRsp[i,j,s]*survdc[i,j,2,s]*kEdc[i,j]	
        TcRsp[i,j,s] <- TdcRsp[i,j,s]*survdc[i,j,2,s]	
     ...
     #   TrW[i,j,s] <- TcW[i,j,s]*survc[i,j,1,s]*kEc[i,j]	
        ...
        TrR[i,j,s]<- TcR[i,j,s]*survc[i,j,2,s]	             #s stands for AU here
        # TrRsp[i,j,s] <- TcRsp[i,j,s]*survc[i,j,2,s]*kEc[i,j]	 
        TrRsp[i,j,s] <- TcRsp[i,j,s]*survc[i,j,2,s] 
        ...
        TspR[i,j,s]  <- TrR[i,j,s]*surv[i,j,5,2]		
        # TspRsp[i,j,s] <- TrRsp[i,j,s]*(1-HrW[i,j])* exp(-2*MR/Tstep) *kE[i,j,5]		
        TspRsp[i,j,s] <- TrRsp[i,j,s]*(1-HrW[i,j])* exp(-2*MR/Tstep)		
        }			
    }
}
...
MR ~dlnorm(-2.3, 4.3)T(0.025,0.35)
  ...
  for(i in 1:(m)){
    ...
    #RMps[i]~dbeta(Ra,Rb) 
    lRMps[i] ~ dnorm(-0.9, 1.77^-2)
    RMps[i] = ilogit(lRMps[i])
...
#     surv[i,1,6,2]<-(1-HtR[i,1]) * exp(-MpsR[i]/Tstep)	
    ...
    for(j in 1:5){
      ...
      logit_MpsR[i,j] = (alpha_mps[j]+delta_mps[i])
      MpsR[i,j] <- ( -log((exp(logit_MpsR[i,j])/(1+exp(logit_MpsR[i,j]))))) * ReffectMps[i]
      
      survMpsR[i,j]<-exp(-MpsR[i,j])             
  
      surv[i,1,1,(j+1)]<-exp(-MpsR[i,j]/Tstep) 	            #survmortR
      
      surv[i,1,2,(j+1)] <- exp(-qlR[i,1]*El[i,1]) * exp(-2*MpsR[i,j]/Tstep)          #survlR
      
  	  surv[i,1,4,(j+1)] <- exp(-qdR[i,1]*Edo[i,1]) * exp(-MpsR[i,j]/Tstep) 	 
       
      surv[i,1,5,(j+1)]<-(1-HrR[i,1])*exp(-MpsR[i,j]*sealMort[i,1,1]/Tstep)    #survrR
      
      surv[i,1,6,(j+1)]<-(1-HtR[i,1]) * exp(-MpsR[i,j]/Tstep)	
    ...
    for (au in 1:3){	
        ...
        survc[i,1,2,au] <-exp(-F_sea[i,1,2,au]) * exp(-(6*MpsR[i,(au)]/Tstep))   
    ...
    survc[i,1,2,4] <-exp(-(6*MpsR[i,4]/Tstep))      #reared 
...
tauL[4]~dlnorm(1.4,46)
  ...
  for(s in 1:stocks){
    ...
    for(j in 1:2){  
          ...
          logit_CV_spawn[s,j]~dnorm(mu_CV_sp[s,j],tau_CV_sp[s,j])  
    ...
    for(i in 1:prior_change[s]){
      logit_pdetect[i,s]~dnorm(logit_mu_spawn[s,1],tau_spawn[s,1])
      ...
      logit_pdetect[i,s]~dnorm(logit_mu_spawn[s,2],tau_spawn[s,2])
      logit(p.detect[i,s])<-logit_pdetect[i,s]
